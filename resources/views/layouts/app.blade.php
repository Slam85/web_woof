...
<body class="bg-secondary text-black bg-opacity-10 h-100 mb-3 pb-3">
    ...
    <header>
        ...
        <nav class="navbar fixed-top text-white bg-danger sticky-top p-2">
            ...
            @if (Route::has('login'))
            @auth
            <h3 class="d-flex justify-content-left align-items-center"><span><img class="logonav m-1" style="border-radius:10px;" src="{{ asset('storage/images/')}}/{{Auth::user()->uuid}}.jpg " />
                </span> Woof,
                {{Auth::user()->username}}!
            </h3>

            <span>
                <a href="{{route ('posts.create')}}"><button type="button" class="btn btn-outline-light">Create Post</button></a>
                <a href="{{route ('index')}}"><button type=" button" class="btn btn-outline-light">My Posts</button></a>
                <a href="{{route ('user.edit')}}"><button type=" button" class="btn btn-outline-light">Profile</button></a>
                <a href="{{route ('deconnexion')}}"><button type="button" class="btn btn-outline-light">LogOut</button></a>
                @else
